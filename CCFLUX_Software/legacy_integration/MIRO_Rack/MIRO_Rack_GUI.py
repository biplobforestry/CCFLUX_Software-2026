...
def absent_meta():
    """The metadata block for an analyzer this flight did not carry.

    The same keys the loaders return, so the page reads zero rows and no gases
    instead of tripping over a missing object.
    """
    return {"files_found":0,"files_used":0,"duplicate_files":[],"skipped_files":[],
            "duplicate_timestamps_removed":0,"sorted":True,"rows":0,
            "start":None,"end":None,"gases":[]}


def load_worker(miro_path, picarro_path):
    """Load whichever analyzers this flight carried.

    A flight can fly one of the two. Loading both unconditionally meant an
    absent MIRO folder raised out of the loader and failed the whole job, so a
    Picarro-only flight could not be loaded at all.
    """
    ...
    miro_path,picarro_path = str(miro_path or "").strip(),str(picarro_path or "").strip()
    if not miro_path and not picarro_path:
        raise RuntimeError("Select a MIRO folder, a Picarro folder, or both.")
    mdata,mmeta = None,absent_meta()
    pdata,pmeta = None,absent_meta()
    if miro_path:
        mdata,mmeta=miro.load_folder(miro_path,miro_progress)
        set_job(.47,"MIRO complete; starting Picarro")
    else:
        add_log("INFO", "loading", "No MIRO folder was selected; loading Picarro alone.")
    if picarro_path:
        pdata,pmeta=picarro.load_folder(picarro_path,picarro_progress)
    else:
        add_log("INFO", "loading", "No Picarro folder was selected; loading MIRO alone.")
    with LOCK:
        STORE["miro"],STORE["picarro"] = mdata,pdata
        STORE["meta"]={"miro":mmeta,"picarro":pmeta,"paths":{"miro":miro_path,"picarro":picarro_path}}
        STORE["results"]=None
        STORE["project"]=None
    for instrument, path, metadata in (("MIRO", miro_path, mmeta), ("Picarro", picarro_path, pmeta)):
        if not path: continue
        add_log("INFO", "loading", f"Loaded {instrument}: {metadata.get('rows', 0):,} rows from {metadata.get('files_used', 0)} files")
...
// values may be absent: a flight can carry one rack instrument and not the
// other, and then that instrument has no gas list to offer.
function fillSelect(select,values,preferred){const list=Array.isArray(values)?values:[];select.innerHTML='';list.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;select.appendChild(o)});if(list.includes(preferred))select.value=preferred}
...
async function restoreSession(){updateSetupState();try{const meta=await getJsonWithRetry('/api/meta',5);if(meta?.miro?.rows||meta?.picarro?.rows){app.meta=meta;miroPath.value=meta.paths?.miro||'';picarroPath.value=meta.paths?.picarro||'';fillSelect(miroGas,meta.miro?.gases,'NO2 wet');fillSelect(picarroGas,meta.picarro?.gases,'CO2 raw');resetFilters();enableLoadedControls();metaSummary('Loaded session | ');await restoreLoadedResults()}}catch(error){console.warn('Could not restore loaded session',error);reportClientError('session restore',error)}}
// Whichever of the two the flight carries. Requiring both left a Picarro-only
// flight with an empty page: no gas list, no controls and no plots, while
// /api/results held a complete Picarro analysis all along.
async function restoreLoadedResults(){let result;try{result=await getJsonWithRetry('/api/results',2)}catch(error){return}const hasMiro=Boolean(result?.miro?.series?.time?.length);const hasPicarro=Boolean(result?.picarro?.series?.time?.length);if(!hasMiro&&!hasPicarro)return;if(hasMiro)await renderMiro(result.miro);if(hasPicarro)await renderPicarro(result.picarro);app.resultsCurrent=true;if(result.comparison&&['CO2','CH4','H2O'].every(gas=>result.comparison[gas])){for(const gas of ['CO2','CH4','H2O'])await renderComparison(gas,result.comparison[gas]);app.comparisonCurrent=true}else resetComparisonPlots(hasMiro&&hasPicarro?'Analyses are ready. Click Proceed to calculate correlations.':'Only one rack instrument was recorded on this flight, so there is nothing to correlate.');updateSetupState()}
