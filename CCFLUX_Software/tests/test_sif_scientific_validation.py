...
# The product comparisons below are retired. This reference was generated by an
# earlier build of the Python pipeline, and comparing it against the R output
# the campaign owner supplied (the same AirFloX raw files, 2024-08-28) showed
# the reference itself is wrong: solar zenith angle out by 0.22 degrees,
# retrieved SIF out by up to 0.70 mW m-2 nm-1 sr-1, and 126 of 448 timestamps
# replaced by interpolated ones. Holding the pipeline to it would lock those
# errors in. test_sif_r_reference_validation.py checks against R instead.
#
# The reference is not deleted: it is the campaign owner's data, and it is worth
# keeping to document what changed. Regenerate it from the corrected pipeline
# before turning these back on.
SUPERSEDED_BY_R_REFERENCE = pytest.mark.skip(
    reason=(
        "Superseded: this reference predates the AIRFLOX_GUI_30.9.R comparison "
        "and encodes defects the R validation exposed. See "
        "test_sif_r_reference_validation.py."
    )
)
